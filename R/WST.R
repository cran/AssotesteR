...
function(y, X, perm=100)
{
	...
	if (sum(complete.cases(X)) != nrow(X))
	    stop("Sorry =(   No missing data allowed in argument 'X'")
    if (nrow(X) != length(y))
    ...
    res = list(wst.stat=wst.stat, asym.pval=asym.pval, perm.pval=perm.pval, args=arg.spec, name=name)
	class(res) = "assoctest"
	return(res)
}
